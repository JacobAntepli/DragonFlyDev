...
void TestObject::printKey(const df::EventKeyboard* p_e)
{
    switch (p_e->getKey()) {
...
void TestObject::printMSE(const df::EventMouse* p_e)
{
...
void TestObject::printCollision(const df::EventCollision* p_e)
{
    printf("COLLISION OCCURED BETWEEN OBJECT WITH ID %d and %d", p_e->getObject1()->getId(), p_e->getObject1()->getId());
}
